...
await page.click('#bSave');await page.waitForTimeout(200);await page.click('#svOk');await page.waitForTimeout(800);await page.click('#svGo');await page.waitForTimeout(3000);
out=await page.evaluate(async id=>{const keys=await new Promise(res=>{const r=indexedDB.open('trace-kv',1);r.onsuccess=()=>{const g=r.result.transaction('kv','readonly').objectStore('kv').getAllKeys();g.onsuccess=()=>res(g.result);};});const s=await new Promise(res=>{const r=indexedDB.open('trace-kv',1);r.onsuccess=()=>{const g=r.result.transaction('kv','readonly').objectStore('kv').get('trace:handoff:'+id);g.onsuccess=()=>res(g.result);};});return {selected:document.querySelector('#siteSel').value,opts:[...document.querySelector('#siteSel').options].map(o=>o.textContent),els:document.querySelectorAll('#net g.el').length,keys:keys.filter(k=>String(k).startsWith('trace:handoff')).length,savedAt:s&&s.traceur&&s.traceur.savedAt,hiddenAt:localStorage.getItem('trace:hiddenAt')};},siteId);console.log('réapparu:',JSON.stringify(out));
