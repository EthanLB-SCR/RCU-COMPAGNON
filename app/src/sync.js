...
export const sync = {
  ...
  async saveWeld(siteId, j) { if (!(await ok())) return false; const { photos, ...d } = j; const { error } = await sb.from('welds').upsert({ site_id: siteId, weld_id: j.weldId, line_id: j.line, cond: j.cond, status: j.status, data: { events: (j.events || []).map(e => ({ ...e, photos: (e.photos || []).map(p => typeof p === 'string' ? p : (p.url || null)).filter(Boolean) })), conn: j.conn, wire: j.wire, loopA: j.loopA || false, loopB: j.loopB || false, tee: j.tee || null, cont: j.cont, iso: j.iso, isoVal: j.isoVal, note: j.note, steps: j.steps || null, photos: (photos || []).map(p => typeof p === 'string' ? p : (p.url || null)).filter(Boolean) }, updated_at: new Date().toISOString() }); if (error) { console.warn(error); return false } return true },
  ...
  async uploadDoc(siteId, folder, name, blob) { if (!(await ok())) return null; try { const safe = String(name||'doc').replace(/[^\w.\-]+/g, '_').slice(-80); const path = `${siteId}/${folder}/${Date.now()}_${safe}`; const { error } = await sb.storage.from('photos').upload(path, blob, { contentType: blob.type || 'application/octet-stream' }); if (error) { console.warn(error); return null } const { data } = await sb.storage.from('photos').createSignedUrl(path, 60 * 60 * 24 * 365 * 3); return data ? data.signedUrl : null } catch (e) { console.warn(e); return null } },
