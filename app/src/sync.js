...
export const sync = {
  ...
  async siteStats() { if (!(await ok())) return null; try { const { data, error } = await sb.rpc('site_stats'); if (error) { return null } const m = {}; (data || []).forEach(r => { m[r.site_id] = { total: r.total, manch: r.manch, last: r.last } }); return m } catch (e) { return null } },
