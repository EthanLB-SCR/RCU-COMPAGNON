...
function renderBouclage(){const el=$('#bouclage');const L=state.locate;const mains=Object.values(state.lines).filter(l=>!l.parent||['A','R'].some(c=>l.cond[c]&&antennaMode(l.id,c).mode==='boucle'));if(!mains.length){el.innerHTML='<h2 class="vt">DH — détection d\'humidité</h2><div class="card muted">Aucune ligne dans ce chantier — trace un réseau avec le traceur.</div>';return;}if(!mains.find(l=>l.id===L.line))L.line=mains[0].id;const line=state.lines[L.line];
  ...
  const mesRows=(dd.mesures||[]).slice(-5).reverse().map(m2=>`<tr><td>${esc(new Date(m2.at).toLocaleDateString('fr-FR'))}</td><td>${esc(m2.weldId||'')}</td><td>${m2.loop?fmt2(m2.loop)+' Ω':'—'}</td><td>${m2.iso!==undefined&&m2.iso!==null?fmt2(m2.iso)+' MΩ':'—'}</td><td>${m2.ok===true?'<span style="color:#0ca30c">✓</span>':m2.ok===false?'<b style="color:#d03b3b">⚠</b>':'—'}</td><td class="dim">${esc(m2.by||'')}</td></tr>`).join('');
  el.innerHTML=`<h2 class="vt">DH — détection d'humidité</h2>
   ...
   ${mesRows?`<div class="card"><h3 style="margin-top:0">Dernières mesures enregistrées</h3><table class="rc"><tr><th>Date</th><th>Point</th><th>Boucle</th><th>Isolement</th><th></th><th>Par</th></tr>${mesRows}</table>${(dd.mesures||[]).length>5?`<div class="muted" style="font-size:11px">${dd.mesures.length} mesures au total (les 5 dernières affichées).</div>`:''}</div>`:''}
  ...
  $('#dh-rows').addEventListener('click',e=>{const tr=e.target.closest('tr[data-line]');if(!tr)return;state.dh.at={line:tr.dataset.line,idx:+tr.dataset.idx};state.dh.meas=null;state.dh.iso=null;state.dh.locVal='';renderBouclage();const m=$('#dh-meas');if(m)m.scrollIntoView({block:'center',behavior:'smooth'});});
  ...
  const svm=$('#dh-save-mesure');if(svm)svm.onclick=()=>{const d2=dhDataOf();if(!d2){toast('Aucun chantier');return;}const at2=state.dh.at;const row2=at2&&dhLoop(L.line,cond).rows.find(r=>r.line===at2.line&&r.idx===at2.idx);
    ...
    d2.mesures.push({at:new Date().toISOString(),by:(me()||{}).name||state.userId,weldId:row2?row2.weldId:'',cond,loop:state.dh.meas||null,iso:isFinite(isoN)?isoN:null,loc:state.dh.locVal||null,ok:okIso===null?undefined:okIso});
    saveDhData();toast('Mesure enregistrée');renderBouclage();};
