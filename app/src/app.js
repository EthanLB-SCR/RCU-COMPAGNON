...
// mini-vue d'orientation : le tronçon COMME SUR LE PLAN, la soudure au centre — de quel côté part l'AMONT, de quel côté l'AVAL
// (Ethan 26/08 : « une vue qui t'indique brièvement de quel côté part l'amont » — la coupe met toujours l'amont à gauche, le terrain non)
function amontAvalSVG(l,c,j){try{const e=l.cond[c].els[j.idx];if(!e)return '';const m=e.m1;const len=l.length||((l.cond[c].els[l.cond[c].els.length-1]||{}).m1||m);
  const M0=Math.max(0,m-9),M1=Math.min(len,m+9);const pts=[];for(let mm=M0;mm<=M1+.001;mm+=Math.max(.8,(M1-M0)/14)){const p=posAtChainage(l,Math.min(M1,mm));if(p&&isFinite(p.x))pts.push(p);}
  if(pts.length<2)return '';const pm=posAtChainage(l,m);const pp=posAtChainage(l,Math.max(0,m-.8));
  ...
  const pad=Math.max(2.5,(x1-x0)*.3,(y1-y0)*.3);x0-=pad;y0-=pad*1.25;x1+=pad;y1+=pad*1.25;const vw=x1-x0,vh=y1-y0;const S9=Math.max(vw,vh);
  const tx=(pm.x-pp.x),ty=(pm.y-pp.y);const tL=Math.hypot(tx,ty)||1;const ux9=tx/tL,uy9=ty/tL;
  const fs=S9*.105;const pA=pts[0],pB=pts[pts.length-1];
  const lab=(p,t,col)=>`<text x="${p.x}" y="${p.y-S9*.06}" font-size="${fs}" font-weight="800" text-anchor="middle" fill="${col}" stroke="#fff" stroke-width="${fs*.32}" paint-order="stroke" font-family="system-ui,sans-serif">${t}</text>`;
  return `<div style="margin:0 0 6px"><svg viewBox="${x0} ${y0} ${vw} ${vh}" style="width:100%;max-height:92px;display:block;background:#edece6;border-radius:8px"><defs><marker id="amvArr" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="4.2" markerHeight="4.2" orient="auto"><path d="M0 0 L10 5 L0 10 Z" fill="#eb6834"/></marker></defs>
   <path d="${pts.map((p,i)=>(i?'L':'M')+p.x.toFixed(2)+' '+p.y.toFixed(2)).join(' ')}" stroke="#3a3d42" stroke-width="${S9*.05}" fill="none" stroke-linecap="round" stroke-linejoin="round"/>
   <path d="M ${(pm.x-ux9*S9*.14).toFixed(2)} ${(pm.y-uy9*S9*.14).toFixed(2)} L ${(pm.x+ux9*S9*.14).toFixed(2)} ${(pm.y+uy9*S9*.14).toFixed(2)}" stroke="#eb6834" stroke-width="${S9*.018}" fill="none" marker-end="url(#amvArr)"/>
   <circle cx="${pm.x}" cy="${pm.y}" r="${S9*.032}" fill="#fff" stroke="#eb6834" stroke-width="${S9*.016}"/>
   ${lab(pA,'AMONT','#1c3d6b')}${lab(pB,'AVAL','#8a4c1e')}${lab({x:pm.x,y:pm.y+S9*.16},esc(j.weldId),'#52514e')}</svg>
   <div class="hint" style="margin:2px 0 0;text-align:center">Orienté comme le PLAN : sur place, l'amont (${esc(e.id)}) est du côté « AMONT » ci-dessus — la coupe en dessous le met toujours à gauche.</div></div>`;}catch(e9){return '';}}
...
function collectStep(j,n){const s=j.steps[n]=j.steps[n]||{photos:[]};s.photos=s.photos||[];const q=id=>$('#'+id,sheetEl);
  ...
  return s;}
sheetEl.addEventListener('click',e=>{const b=e.target.closest('[data-act],[data-rot],[data-sw],[data-wire],[data-rotb],[data-flipb],[data-rota],[data-flipa],[data-saut],[data-teedown],[data-dhend],[data-stepok],[data-stepundo],[data-stkpick],[data-dhfrz],[data-gostep]');if(!b)return;const s=state.sel;if(!s)return;const l=state.lines[s.line];
