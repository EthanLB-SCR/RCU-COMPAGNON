...
const WIRE={E:{label:'fil étamé (alarme)',short:'étamé',color:'#dfe4ea',clock:300},N:{label:'fil cuivré (retour)',short:'cuivré',color:'#e2843a',clock:60}};
...
function wipeWeld(j){j.status='a_souder';j.events=[];j.photos=[];j.wire='a_raccorder';j.conn={E:'E',N:'N'};j.cont=false;j.iso=false;j.isoVal='';j.note='';delete j.loopA;delete j.loopB;delete j.steps;try{stockDropTakes(j.weldId);}catch(e){}}
...
const TFIELDS=['status','events','photos','wire','conn','loopA','loopB','cont','iso','isoVal','note','steps']; // 'steps' : les 4 sous-étapes du manchon suivent le transfert (photos, gars, dates, bouclage figé)
...
function recordElPos(l,c,el){const m=elPosOf();if(!m||!l||!el)return;const k2=l.id+'|'+c+'|'+el.id;
  if(el.rot||el.flip||el.teeDown)m[k2]={rot:el.rot||0,flip:el.flip?1:0,td:el.teeDown?1:0};else delete m[k2];saveElPos();}
function applyElPos(){const m=NET&&NET.elPos;if(!m)return;Object.entries(m).forEach(([k2,v])=>{const [lid,c,eid]=k2.split('|');const l=state.lines[lid];const cd=l&&l.cond&&l.cond[c];if(!cd)return;const el=cd.els.find(x=>x.id===eid);if(el){el.rot=v.rot||0;el.flip=!!v.flip;el.teeDown=!!v.td;}});}
let netPartT={};
...
function rtSubscribe(id){if(state.rtOff){try{state.rtOff();}catch(e){}state.rtOff=null;}if(!id||id==='__vide')return;
  state.rtOff=sync.subscribeSite(id,{
    onWeld:row=>{if(!row||!row.weld_id)return;const f=findWeld(row.weld_id);if(!f)return;const j=f.j;j.status=row.status||j.status;const d=row.data||{};if(d.events)j.events=d.events.map(e=>({...e,at:new Date(e.at)}));if(d.conn)j.conn=d.conn;if(d.wire)j.wire=d.wire;if(d.loopA!==undefined)j.loopA=d.loopA||undefined;if(d.loopB!==undefined)j.loopB=d.loopB||undefined;if(d.tee!==undefined)j.tee=d.tee||undefined;if(d.cont!==undefined)j.cont=d.cont;if(d.iso!==undefined)j.iso=d.iso;if(d.isoVal!==undefined)j.isoVal=d.isoVal;if(d.note!==undefined)j.note=d.note;if(d.photos)j.photos=d.photos;scheduleRender();if(state.tab!=='plan')renderAll();},
...
function renderPlan(){if(typeof linkTraceurBranches==='function')linkTraceurBranches(); // e.branchLine posé : le dessin du té sait quelle antenne s'y raccorde (mode série / bouclée)
  ...
  sh.lines.forEach(id=>{const line=state.lines[id];['A','R'].forEach(c=>{if(!line.cond[c])return;const side=c==='A'?1:-1;const col=c==='A'?'#c8382f':'#2a5fb4';const {els,joints}=line.cond[c];
    els.forEach((e,i)=>{if(cull&&(e.bbox[2]<vx0||e.bbox[0]>vx1||e.bbox[3]<vy0||e.bbox[1]>vy1))return;
      ...
      const detail=kpm>=12; const Lax=e.len; const nueEl=!!e.nue||(e.manchette&&Lax<=0.35); const bare=(detail&&e.kind!=='valve'&&e.kind!=='endcap'&&Lax>0.36)?.15:0; // bouts d'acier nus (15 cm) au zoom près ; manchette nue : tout en acier
      ...
      const isSteel=e.kind==='steelbend'||(e.kind==='bypass'&&!e.link); // courbe acier (SXB) — et le BY-PASS : 2 coudes 90° recoupés en ACIER NU, isolés sur place (Ethan 26/08)
      ...
      if(far){ // loin : un trait par conduite, aller rouge / retour bleu, teinté par l'avancement de la soudure aval
        ...
        e.axis.forEach((pl,pi)=>{const isMain=pi===0;const LaxR=polyLen(pl);const m0=isMain?bare:0,m1=isMain?LaxR-bare:LaxR;const cas=offsetPoly(axisSub(pl,m0,m1),d); // longueur RÉELLE de la polyline : un by-pass a un arc bien plus long que son emprise e.len (Ethan 26/08 : le U était invisible)
          ...
          if(isMain&&bare){[[0,'L'],[1,'R']].forEach(([side2,_])=>{const mS=side2?LaxR-bare:0,mE=side2?LaxR:bare;const st2=offsetPoly(axisSub(pl,mS,mE),d);const stroke=(side2?jB:jA)&&(side2?jB:jA).status==='manchonnee'?null:'#aeb4bb';
            if(stroke){net+=`<path d="${pathD(st2)}" stroke="${stroke}" stroke-width="${steelW}" fill="none" stroke-linecap="butt"/><path d="${pathD(offsetPoly(axisSub(pl,mS,mE),d-steelW*.3))}" stroke="#e6e9ed" stroke-width="${steelW*.22}" fill="none" opacity=".6"/>`;
              const mF=side2?LaxR-bare:bare;const t2=axisSub(pl,Math.max(0,mF-.03),Math.min(LaxR,mF+.03));const q=offsetPoly(axisSub(pl,mF,mF+.001),d)[0];const dx=t2[t2.length-1].x-t2[0].x,dy=t2[t2.length-1].y-t2[0].y;const L2=Math.hypot(dx,dy)||1;const nx=-dy/L2*w/2,ny=dx/L2*w/2;
              net+=`<line x1="${q.x-nx}" y1="${q.y-ny}" x2="${q.x+nx}" y2="${q.y+ny}" stroke="#e3cd63" stroke-width="${Math.max(2/k,foamT)}" stroke-linecap="butt"/>`;}});}
          if(isMain&&e.kind==='pipe'&&LaxR>3){const colr=line.single?(line.single==='A'?'#c8382f':'#2a5fb4'):col;for(let m=bare+.7*casingOf(e)*EX;m<LaxR-bare-.5*casingOf(e)*EX;m+=2.5){net+=`<path d="${pathD(offsetPoly(axisSub(pl,m,m+.16*casingOf(e)*EX),d))}" stroke="${colr}" stroke-width="${w}" fill="none" stroke-linecap="butt"/>`;}}
        });
      ...
      if(showWires&&S.fils&&e.kind!=='valve'&&e.kind!=='endcap'&&!isSteel){
        ...
        ['E','N'].forEach(wn=>{const a=brT?wireGOf(e,wn):clockPos(e,wn);let o=(-Math.sin(rad(a)))*casingOf(e)*EX*.37*ppm;let front=Math.cos(rad(a))>=0; // té : positions RÉELLES de la pièce, 10 h/2 h (⤓ retourné : 8 h/4 h, fils dessous → pointillé) — même chose que la coupe de la fiche (Ethan 26/08)
          ...
          if(brT&&wn===wbT){ // ce fil PLONGE dans le té, il ne traverse pas tout droit (maquette validée 25/08)
            ...
            const outJ2=child&&state.lines[child].cond[c]&&state.lines[child].cond[c].joints[0];
            const abL2=(kpm>=12&&uL>0.5&&!(outJ2&&outJ2.status==='manchonnee'))?.24:0; // bague (0,09) + acier (0,15) dessinés au bout de la branche
            const tip=mode3==='none'?Math.min(uL*.45,uL-.2):Math.max(abL2+.02,Math.min(.12,uL*.06)); // les fils s'arrêtent AU TRAIT JAUNE de la mousse, jamais sur la bague ou l'acier (Ethan 26/08)
            ...
            const mk6=(A,sg)=>{if(!A)return null;const tU=Math.min(Math.max((A.x-b0.x)*uxn+(A.y-b0.y)*uyn,gap2*.6),uL*.55); // raccord DANS le corps du té, même quand le fil arrive du côté opposé (il traverse la mousse — Ethan 26/08 : « les fils dépassent du calo »)
      ...
      if(showElLabels&&(S.pieces||S.cotes)&&(c==='A'||line.single)){const mid=elMid(e);const lp=e.kind==='teeout'?{x:mid.x+mid.tx*(w*1.6)+mid.ty*d*1.4,y:mid.y+mid.ty*(w*1.6)-mid.tx*d*1.4}:{x:mid.x+mid.ty*d*2.6,y:mid.y-mid.tx*d*2.6};const fs=Math.max(10/k,Math.min(.3*ppm,17/k));
        ...
        const isPB=e.kind==='pipe'||e.kind==='bend'||e.kind==='steelbend';const name=(S.pieces&&!e.link)?(isPB?e.id:`${e.id} · ${e.kindLabel||e.kind}`)+((e.rot||e.flip)?' ↻'+e.rot+'°':''):'';const cote=S.cotes?(e.kind==='pipe'?`${fmt(e.len)} m`:(e.kind==='bend'||e.kind==='steelbend')&&e.angle?`${e.angle}°`:''):'';const lab=[name,cote].filter(Boolean).join(' · ');
    ...
    joints.forEach((j,i)=>{const e=els[i];if(cull&&(e.to.x<vx0||e.to.x>vx1||e.to.y<vy0||e.to.y>vy1))return;const p=jointPos(line,i,c);const w=casingW(e);const d=(line.single?0:e.ownAxis?Math.max(0,(far?minSep/(k*ppm):(w*k+2)/(2*k*ppm))-(line.axisHalf||.2))*side:Math.max(offM(e),far?0:(w*k+2)/(2*k*ppm))*side)*ppm;const px=p.x+p.ty*d,py=p.y-p.tx*d;
      ...
      if(detail){
        ...
        } else if(j.status!=='a_souder'&&j.status!=='manchonnee'){ // cordon de soudure : bourrelet sombre, cœur clair — l'acier-SOUDURE-acier se lit (Ethan 26/08 « tout collé »)
          netJ+=`<path d="${pathD(seg(-.05,.05))}" stroke="#42474d" stroke-width="${w*.62}" fill="none" stroke-linecap="butt"/><path d="${pathD(seg(-.032,.032))}" stroke="#6a7078" stroke-width="${w*.58}" fill="none" stroke-linecap="butt"/><path d="${pathD(seg(-.012,.012))}" stroke="#949aa2" stroke-width="${w*.52}" fill="none" stroke-linecap="butt"/>`;
...
function openJoint(lineId,c,i){state.sel={kind:'j',line:lineId,cond:c,i};state.sheetMode='view';resetForm();const j=state.lines[lineId].cond[c].joints[i];state.conn={...j.conn};state.loopA=!!j.loopA;state.loopB=!!j.loopB;state.wsel=null;state.wselB=null;state.sw={etanch:false,mousse:false,cont:j.cont,iso:j.iso};ensureVisible(lineId,c,i);renderSheet();openUI();renderPlan();}
...
function jointView(l,c,j){const {els}=l.cond[c];const a=els[j.idx],b=els[j.idx+1];const r=role();const st=j.status;const acts=[];const evs=j.events.slice().reverse();
  ...
  const teeUI=j.teeOut?(()=>{const t=j.tee||{};const dw=wireDefFournisseur()==='N'?'cuivré':'étamé';return `<div class="card" style="margin-top:8px;background:#f4f7fb"><b>Antenne au té</b> <span class="muted" style="font-size:12px">(sortie de té : comment l'antenne est prise dans la boucle DH)</span><div class="row" style="margin-top:6px;display:flex;gap:6px;flex-wrap:wrap"><select class="f" id="teeMode"><option value="serie" ${(t.mode||'serie')==='serie'?'selected':''}>en série dans la boucle de la parente</option><option value="boucle" ${t.mode==='boucle'?'selected':''}>bouclée sur elle-même au té (boucle à part)</option><option value="none" ${t.mode==='none'?'selected':''}>pas encore raccordée au té</option></select><span class="hyChip" style="align-self:center" title="Règle fournisseur, non modifiable : Renalia / AXIOM = cuivré, LOGSTOR = étamé.">fil dans l'antenne : <b>${dw}</b></span></div></div>`;})():'';
  const wiringRO=(j.wire==='raccorde'||j.wire==='inversion')?`<div class="card" style="padding:6px 8px 2px">${amontAvalSVG(l,c,j)}${wiringSVG(a,b,j.conn||{E:'E',N:'N'},null,false,{A:j.loopA,B:j.loopB})}</div>`:'';
  const wireLine=(teeUI)+wiringRO+((j.loopA||j.loopB)?`<div class="okbox" style="background:#f3ecff;border-color:#c9b3ea">⟲ Fils bouclés ENTRE EUX ${j.loopA?'côté AMONT':''}${j.loopA&&j.loopB?' et ':''}${j.loopB?'côté AVAL':''} dans ce manchon — la boucle DH se ferme ici. Continuité ${j.cont?'OK':'—'}, isolement ${j.iso?'OK'+(j.isoVal?' ('+esc(j.isoVal)+' MΩ)':''):'—'}.</div>`:j.wire==='raccorde'?`<div class="okbox">Fils raccordés (${a.kind==='teeout'?'sortie de té : brins du té → fils de l\'antenne':'étamé ↔ étamé, cuivré ↔ cuivré'}), continuité ${j.cont?'OK':'—'}, isolement ${j.iso?'OK'+(j.isoVal?' ('+esc(j.isoVal)+' MΩ)':''):'—'}.</div>`:j.wire==='inversion'?`<div class="err">Inversion des fils enregistrée : étamé amont → cuivré aval. Manchon à reprendre avant fermeture. ${esc(j.note)}</div>`:`<p class="hint">Fils d'alarme : à raccorder au manchonnage.</p>`);
  ...
  if(frz){const okL=frz.meas&&frz.expected?Math.abs(Math.round(100*(frz.meas-frz.expected)/frz.expected))<=(frz.tol||5):null;
    att2=`<div class="${okL===false?'warnbox':'okbox'}" style="margin:4px 0;font-size:12.5px;cursor:pointer" data-dhfrz="1" title="afficher ce bouclage sur le plan">
     ...
     boucle ${esc(frz.dir||'')} fermée par <b>${esc(frz.closure||'—')}</b> · attendu <b>${frz.expected?fmt2(frz.expected)+' Ω':'—'}</b>${frz.dE!==undefined?` <span class="dim">(étamé ${fmt(frz.dE)} + cuivré ${fmt(frz.dN)} m · ${esc(frz.rkm)} Ω/km)</span>`:''}<br>
  ...
  const stepsCard=`<h3>Manchon — 4 sous-étapes <span class="muted" style="font-weight:400">· ${nDone}/4${nDone===4?' ✓':''}</span></h3>
   ...
   <details class="dstep ${sdone(2)?'done':curStep===2?'cur':''}" ${curStep===2?'open':''}>${sHead(2,'Fils raccordés')}<div class="sb">${att2}
    ${sdone(2)?`<div class="card" style="padding:6px 8px 2px">${amontAvalSVG(l,c,j)}${wiringSVG(a,b,j.conn||{E:'E',N:'N'},null,false,{A:j.loopA,B:j.loopB})}</div>`
      :`<div class="muted" style="font-size:11.5px;margin:4px 0">Relie les fils : touche un bout de fil du tube amont (à gauche) puis celui d'en face qu'il rejoint (ou « ∅ non raccordé »). Pour BOUCLER les deux fils d'un même tube entre eux (isoler un côté), touche ses DEUX fils l'un après l'autre.${state.wsel?` <b>${aTeeJ?'Brin '+(state.wsel==='E'?'AMONT':'AVAL')+' du té choisi':'Fil '+(state.wsel==='E'?'étamé':'cuivré')+' amont choisi'} → touche le fil aval qu'il rejoint, ou l'autre fil amont pour les boucler entre eux.</b>`:''}${state.wselB?` <b>Fil ${state.wselB==='E'?'étamé':'cuivré'} aval choisi → touche l'autre fil aval pour les boucler entre eux.</b>`:''}</div>
       <div class="card" style="padding:6px 8px 2px">${amontAvalSVG(l,c,j)}${wiringSVG(a,b,state.conn,state.wsel,true,{A:state.loopA,B:state.loopB,selB:state.wselB})}</div>${rotCtlHTML(a,'a','amont',lockA)}${rotCtlHTML(b,'b','aval',lockB)}${wireRow2('E')}${wireRow2('N')}
       ${(state.loopA||state.loopB)?'<div class="okbox" style="font-size:12px;background:#f3ecff;border-color:#c9b3ea">⟲ Fils bouclés ENTRE EUX '+(state.loopA?'côté AMONT':'')+(state.loopA&&state.loopB?' et ':'')+(state.loopB?'côté AVAL':'')+' : la boucle DH se ferme à ce manchon.</div>':aTeeJ?'<div class="okbox" style="font-size:12px">Sortie de té : les deux brins du fil '+(wireOfTee(a)==='E'?'étamé':'cuivré')+' du té rejoignent les deux fils de l\'antenne — pas d\'inversion possible ici.</div>':(state.conn.E==='N'||state.conn.N==='E')?'<div class="err" style="font-size:12px">Inversion : l\'étamé amont part sur le cuivré aval — enregistre si c\'est ce qui a été fait.</div>':'<div class="okbox" style="font-size:12px">Raccordement droit : étamé ↔ étamé, cuivré ↔ cuivré.</div>'}`}
...
function stockPickMapSVG(zs,p,mode){const s=stockOf();if(!s)return '';const zsAll=s.zones.filter(z=>z.status==='ok');if(!zsAll.length)return '';
  ...
  const pad=Math.max(8,(x1-x0)*.14,(y1-y0)*.14);x0-=pad;y0-=pad*1.7;x1+=pad;y1+=pad;const vw=x1-x0,vh=y1-y0;
  let out='',hitsZ='',shapes='';
  ...
  const fs=Math.max(2.2,vw*.032);const labs=[];
  zsAll.forEach(z=>{const has=okIds.has(z.id);const on=z.id===pre;const tiny=Math.max(z.w,z.h)<vw*.05; // stockages éloignés : zone minuscule à cette échelle
    if(has)hitsZ+=`<circle cx="${z.x}" cy="${z.y}" r="${Math.max(vw*.045,Math.max(z.w,z.h)*.75)}" fill="transparent" data-stkpick="${z.id}" data-stkneed="${mode}" data-stkhit="1" ${on?'data-on="1"':''} style="cursor:pointer"/>`;
    shapes+=`<g transform="translate(${z.x} ${z.y}) rotate(${z.rot||0})"><rect x="${-z.w/2}" y="${-z.h/2}" width="${z.w}" height="${z.h}" rx="1" fill="${has?'#0ca30c':'#8f8b80'}" fill-opacity="${on?'0.5':'0.22'}" stroke="${on?'#eb6834':'#8f8b80'}" stroke-width="${on?2.5:1.2}" vector-effect="non-scaling-stroke" ${has?`data-stkpick="${z.id}" data-stkneed="${mode}" ${on?'data-on="1"':''} style="cursor:pointer"`:''}/></g>`;
    if(tiny)shapes+=`<circle cx="${z.x}" cy="${z.y}" r="${vw*.012}" fill="${has?'#0ca30c':'#8f8b80'}" fill-opacity="${on?'.9':'.55'}" stroke="${on?'#eb6834':'#fff'}" stroke-width="1.5" vector-effect="non-scaling-stroke" style="pointer-events:none"/>`;
    labs.push({x:z.x,oy:z.y-(tiny?vw*.02:z.h/2+vw*.012),zx:z.x,zy:z.y-(tiny?vw*.012:z.h/2),t:z.name+(has?'':' — n\'en a pas'),fill:has?'#3c3b36':'#8f8b80',fw:'700',fs});});
  labs.push({x:p.x,oy:p.y-Math.max(2.6,vw*.026),zx:p.x,zy:p.y,t:'la soudure',fill:'#d03b3b',fw:'800',fs:Math.max(2.2,vw*.03)});
  // étiquettes SANS chevauchement : celles qui se marchent dessus s'empilent vers le haut, avec un trait de rappel (Ethan 26/08 : « dézoomé c'est pas lisible »)
  const placed=[];labs.forEach(L=>{L.y=L.oy;const w2=L.t.length*L.fs*.6,h2=L.fs*1.25;let g2=0;
    while(g2++<40&&placed.some(P2=>Math.abs(P2.y-L.y)<(P2.h2+h2)/2&&Math.abs(P2.x-L.x)<(P2.w2+w2)/2))L.y-=h2;
    if(L.y<y0+h2)L.y=y0+h2;placed.push({x:L.x,y:L.y,w2,h2});
    if(L.oy-L.y>h2*.9)out+=`<line x1="${L.x}" y1="${L.y+L.fs*.4}" x2="${L.zx}" y2="${L.zy}" stroke="${L.fill}" stroke-width=".8" vector-effect="non-scaling-stroke" opacity=".55" stroke-dasharray="2 2"/>`;});
  out+=hitsZ+shapes;
  labs.forEach(L=>{out+=`<text x="${L.x}" y="${L.y}" font-size="${L.fs}" text-anchor="middle" font-weight="${L.fw}" fill="${L.fill}" stroke="#f2f1ea" stroke-width="${(L.fs*.28).toFixed(2)}" paint-order="stroke" style="pointer-events:none" font-family="system-ui,sans-serif">${esc(L.t)}</text>`;});
  out+=`<circle cx="${p.x}" cy="${p.y}" r="${Math.max(1.4,vw*.014)}" fill="#d03b3b" stroke="#fff" stroke-width="1.6" vector-effect="non-scaling-stroke"/>`;
  return `<svg viewBox="${x0} ${y0} ${vw} ${vh}" preserveAspectRatio="xMidYMid meet" style="width:100%;max-height:200px;background:#edece6;border:1px solid var(--line);border-radius:9px;display:block;margin:4px 0 6px">${out}</svg>`;}
function stockPickHTML(l,c,j,mode){const s=stockOf();if(!s)return '';
  const okZones=s.zones.filter(z=>z.status==='ok');
  const need=stockNeedOf(l,c,j,mode);if(!need)return '';
  if(!okZones.length)return ((s.livs&&s.livs.length)||(s.lots&&s.lots.length))?`<div class="warnbox" style="font-size:12px">Aucune zone de stockage active sur le plan : ${mode==='piece'?'la pièce':mode==='sleeve'?'le manchon':'la mousse'} ne sera pas décomptée. Décharge un camion ou recrée une zone (onglet Stock).</div>`:'';
  ...
  const has=zs.length>0; // aucune zone n'a la pièce : la question RESTE posée (Ethan 26/08 « ça ne me demande plus ») — on répond quand même, le suivi se corrige
  const list=has?zs:okZones.map(z2=>({z:z2,reste:0,d:(isFinite(p.x)&&isFinite(z2.x))?Math.hypot(z2.x-p.x,z2.y-p.y):1e9})).sort((a,b)=>a.d-b.d);
  return `<h3>${mode==='piece'?'Pièce prise dans quel stockage ?':mode==='sleeve'?'Manchon enfilé sur le tube — pris dans quel stockage ?':'Mousse prise dans quel stockage ?'} <span class="muted" style="font-weight:400;font-size:11.5px">${esc(need.label)} — décompte en direct</span></h3>
   <div class="card" style="padding:8px">${has?'':`<div class="warnbox" style="font-size:12px;margin:0 0 5px">D'après l'appli, aucun stockage n'a « ${esc(need.label)} ». Dis quand même d'où ${mode==='sleeve'?'il':'elle'} vient : le stock de la zone passera en négatif et l'écart se verra (livraison oubliée ?).</div>`}${stockPickMapSVG(zs,p,mode)}<div class="hint" style="margin:0 0 4px">Touche la zone sur la carte, ou une pastille :</div>${list.map((o,i)=>`<span class="hyChip" data-stkpick="${o.z.id}" data-stkneed="${mode}" style="cursor:pointer;margin:2px;${has&&i===0?'border-color:#eb6834;background:#fff7f2':''}" ${has&&i===0?'data-on="1"':''}>${esc(o.z.name)} <span class="dim" style="font-size:10px">${o.d<9e8?'à '+fmt(o.d)+' m · ':''}${o.reste?'reste '+o.reste:"n'en a pas"}</span></span>`).join('')}<span class="hyChip" data-stkpick="none" data-stkneed="${mode}" style="cursor:pointer;margin:2px;${has?'':'border-color:#eb6834;background:#fff7f2'}" ${has?'':'data-on="1"'}>hors stock / ailleurs</span></div>`;}
...
function wiringSVG(a,b,conn,sel,editable,loops){
  const teeIn=a&&a.kind==='teeout'?{wire:wireOfTee(a)}:null; // SORTIE DE TÉ : côté amont, les DEUX BRINS du même fil du té arrivent le long de la branche (pas un étamé + un nu face à face)
  ...
  const exit=(e2,xg,dir,w)=>{const g=wireGOf(e2,w);return [xg-dir*Math.sin(rad(g))*kxm*.9,cy-Math.cos(rad(g))*rr+Math.sin(rad(g))*PL,g];};
  ...
  const exitG=(g,xg,dir)=>[xg-dir*Math.sin(rad(g))*kxm*.9,cy-Math.cos(rad(g))*rr+Math.sin(rad(g))*PL,g];
  const P=teeIn?{aE:exitG(270,xgL,+1),aN:exitG(90,xgL,+1),bE:exit(b,xgR,-1,'E'),bN:exit(b,xgR,-1,'N')}:{aE:exit(a,xgL,+1,'E'),aN:exit(a,xgL,+1,'N'),bE:exit(b,xgR,-1,'E'),bN:exit(b,xgR,-1,'N')};
  ...
  Object.entries(P).forEach(([k,p])=>{const side=k[0],w=k[1];
    ...
    labels+=`<text x="${p[0]+ax*11}" y="${back?p[1]-11:p[1]+19}" font-size="12" text-anchor="${ax<0?'end':'start'}" fill="${(side==='a'&&teeIn?teeIn.wire:w)==='E'?'#5c646c':'#8a4c1e'}" font-weight="600" font-family="system-ui,sans-serif" stroke="#fff" stroke-width="3" paint-order="stroke">${side==='a'&&teeIn?(w==='E'?'brin AMONT':'brin AVAL'):clockText(p[2])}</text>`;});
  const usedB={};const hitPaths=[];const lp=loops||{};const wcolA=w2=>teeIn?wcol(teeIn.wire):wcol(w2);
  // fils bouclés ENTRE EUX (même tube) : pont en U entre les deux sorties de la tranche, serti au milieu (Ethan 26/08)
  const bridgeU=(pE,pN,ax,lab)=>{const mx=(pE[0]+pN[0])/2+ax*46,my=(pE[1]+pN[1])/2;
    filsF+=strokes(`M ${pE[0]} ${pE[1]} Q ${mx} ${pE[1]} ${mx} ${my}`,wcolA('E'),false);
    filsF+=strokes(`M ${pN[0]} ${pN[1]} Q ${mx} ${pN[1]} ${mx} ${my}`,wcolA('N'),false);
    crimpsF+=`<rect x="${mx-4.5}" y="${my-10}" width="9" height="20" rx="3.5" fill="url(#wpsn)" stroke="#6f777f" stroke-width="1"/><line x1="${mx-4.5}" y1="${my-3}" x2="${mx+4.5}" y2="${my-3}" stroke="#868e96" stroke-width="1"/><line x1="${mx-4.5}" y1="${my+3}" x2="${mx+4.5}" y2="${my+3}" stroke="#868e96" stroke-width="1"/>`;
    labels+=`<text x="${mx}" y="${my+34}" font-size="11.5" font-weight="700" text-anchor="middle" fill="#7a3fd1" font-family="system-ui,sans-serif" stroke="#fff" stroke-width="3" paint-order="stroke">⟲ ${lab}</text>`;};
  if(lp.A)bridgeU(P.aE,P.aN,+1,'fils amont bouclés entre eux');
  if(lp.B)bridgeU(P.bE,P.bN,-1,'fils aval bouclés entre eux');
  ['E','N'].forEach((wA,wi)=>{const pA=P['a'+wA];if(lp.A)return;const to=conn[wA];
    if(!to||to==='X'){
      // fil amont non raccordé : petit bout qui retombe + pointillé
      filsF+=strokes(`M ${pA[0]} ${pA[1]} C ${pA[0]+12} ${pA[1]-3} ${pA[0]+22} ${pA[1]+1} ${pA[0]+30} ${pA[1]+7}`,wcolA(wA),editable&&sel===wA);
      filsF+=`<line x1="${pA[0]+34}" y1="${pA[1]+10}" x2="${pA[0]+52}" y2="${pA[1]+15}" stroke="${(teeIn?teeIn.wire:wA)==='E'?'#9aa2aa':'#b96f33'}" stroke-width="3" stroke-dasharray="4 4" stroke-linecap="round"/><text x="${pA[0]+58}" y="${pA[1]+20}" font-size="12" fill="#898781" font-family="system-ui,sans-serif">∅ non raccordé</text>`;
      return;}
    usedB[to]=1;
    const pB=P['b'+to];const bad=teeIn?false:to!==wA;const recol=teeIn?to!==teeIn.wire:bad;const gA=pA[2],gB=pB[2];
    drawSegs(helix(pA,gA,pB,gB,0,1,wi),wcolA(wA),editable&&sel===wA);
    const t0=wi?0.565:0.435;
    // en inversion, la couleur change AU raccord serti : partie aval recolorée
    if(recol)drawSegs(helix(pA,gA,pB,gB,t0,1,wi),wcol(to),false);
  ...
  ['E','N'].forEach(wB=>{if(usedB[wB]||lp.B)return;const pB=P['b'+wB];
  ...
  if(editable&&(lp.selB==='E'||lp.selB==='N')){const p=P['b'+lp.selB];s+=`<circle cx="${p[0]}" cy="${p[1]}" r="12" fill="none" stroke="#7a3fd1" stroke-width="2.5"/>`;}
  ...
  s+=labels;
  const inv=!teeIn&&((conn.E&&conn.E!=='X'&&conn.E!=='E')||(conn.N&&conn.N!=='X'&&conn.N!=='N'));
  ...
  if(lp.A||lp.B)s+=`<g><rect x="${xs-84}" y="4" width="168" height="22" rx="11" fill="#7a3fd1"/><text x="${xs}" y="20" font-size="12" font-weight="700" text-anchor="middle" fill="#fff" font-family="system-ui,sans-serif">⟲ BOUCLÉS ENTRE EUX</text></g>`;
...
const rotCtlHTML=(e2,suf,side,lock)=>e2&&(e2.kind==='teeout'||(e2.kind==='tee'&&Array.isArray(e2.branch)&&e2.branch.length===2))?`<div class="hint" style="margin:2px 0;text-align:center">${e2.id} (${side}) : té — pas de rotation libre, fils à ${e2.teeDown?'8 h / 4 h (retourné)':'10 h / 2 h'}${e2.kind==='teeout'?'':` — le ${wireOfTee(e2)==='E'?'fil étamé':'fil cuivré'} part dans l'antenne`}.${e2.kind==='tee'?` <button class="btn sm ${e2.teeDown?'on':''}" data-teedownj="${suf}" style="margin-left:6px;padding:3px 8px">⤓ ${e2.teeDown?'retourné (fils en dessous) — remettre AU-DESSUS':'té posé à l\'envers ? retourner (fils en dessous)'}</button>`:''}</div>`:lock?`<div class="lockbox">🔒 <b>${e2.id}</b> (${side}) : position figée — déjà manchonnée en ${lock.map(esc).join(' et ')}. Si ce manchon-là est bon, le tube est forcément en bonne position.</div>`
...
// mini-vue d'orientation v2 (Ethan 26/08 soir : « fais mieux — une vue photo de l'endroit, un schéma plus précis du réseau ») :
// PHOTO AÉRIENNE IGN de l'endroit (si le chantier est géoréférencé), tout le réseau alentour (la ligne courante en évidence),
// les manchons voisins NUMÉROTÉS, la soudure au centre, flèche du sens et AMONT/AVAL posés SUR le tracé réel.
function amontAvalSVG(l,c,j,big){try{const e=l.cond[c].els[j.idx];if(!e)return '';const m=e.m1;
  ...
  let tiles='';try{const geo9=siteGeo();if(geo9){const T=tilesFor(geo9,[x0,y0,x0+vw,y0+vh],big?12:14,19,big?60:24);
    if(T&&T.tiles&&T.tiles.length){const xs=T.tiles.map(t=>t.x),ys=T.tiles.map(t=>t.y);const mx=Math.min(...xs),my=Math.min(...ys);
      const ox=mx*256,oy=my*256;const [a2,b2,c2,d2,e2,f2]=T.matrix;const mat=`matrix(${[a2,b2,c2,d2,e2+a2*ox+c2*oy,f2+b2*ox+d2*oy].map(x=>(+x).toPrecision(10)).join(' ')})`;
      tiles=`<g transform="${mat}">${T.tiles.map(t=>`<image href="${ignTileURL('ortho',T.z,t.x,t.y)}" x="${(t.x-mx)*256}" y="${(t.y-my)*256}" width="256.5" height="256.5"/>`).join('')}</g><rect x="${x0}" y="${y0}" width="${vw}" height="${vh}" fill="#fff" opacity=".07"/>`;}}}catch(e8){}
  // réseau alentour : toutes les lignes (halo blanc + trait), la ligne courante en évidence
  let net9='';Object.values(state.lines).forEach(l2=>{const pts=(l2.pts||[]);if(pts.length<2)return;const dd9='M '+pts.map(q=>q.x.toFixed(1)+' '+q.y.toFixed(1)).join(' L ');const cur9=l2.id===l.id;
    net9+=`<path d="${dd9}" stroke="#fff" stroke-width="${cur9?1.5:1.05}" fill="none" stroke-linecap="round" stroke-linejoin="round" opacity=".75"/><path d="${dd9}" stroke="${cur9?'#14161a':'#565b61'}" stroke-width="${cur9?1.0:.62}" fill="none" stroke-linecap="round" stroke-linejoin="round"/>`;});
  // manchons voisins de la ligne courante, numérotés (repères terrain)
  let js9='';{const els9=l.cond[c].els,jj9=l.cond[c].joints;
    for(let i9=0;i9<jj9.length;i9++){const ee=els9[i9];if(!ee)continue;const mm=ee.m1;if(Math.abs(mm-m)>R*1.02||i9===j.idx)continue;const p9=posAtChainage(l,mm);if(!p9||!isFinite(p9.x))continue;
  ...
  const svg9=`<svg viewBox="${x0} ${y0} ${vw} ${vh}" preserveAspectRatio="xMidYMid slice" style="width:100%;height:${big?'56vh':'138px'};display:block;background:#e6e4dc;border-radius:${big?'10px':'8px'}"><defs><marker id="amvArr" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="4.2" markerHeight="4.2" orient="auto"><path d="M0 0 L10 5 L0 10 Z" fill="#eb6834"/></marker></defs>
   ...
   ${pA?lab(pA,'AMONT','#123c78'):''}${pB?lab(pB,'AVAL','#8a4c1e'):''}${lab({x:pm.x,y:pm.y+(big?7:4.6)},esc(j.weldId),'#0b0b0b',big?2.8:1.7)}</svg>`;
  if(big)return svg9;
  return `<div style="margin:0 0 6px" data-amvzoom="1" style2=""><div style="position:relative;cursor:zoom-in">${svg9}<div style="position:absolute;right:6px;bottom:6px;background:rgba(255,255,255,.88);border-radius:7px;padding:2px 7px;font-size:11px;color:#1c3d6b;pointer-events:none">🔍 agrandir</div></div>
   <div class="hint" style="margin:2px 0 0;text-align:center">Orienté comme le PLAN${tiles?' (photo de l\'endroit)':''} — l'amont (${esc(e.id)}) part du côté « AMONT ». Touche la photo pour la vue large.</div></div>`;}catch(e9){return '';}}
function formManchon(l,c,j){const {els}=l.cond[c];const a=els[j.idx],b=els[j.idx+1];const lwF=!!(state.loopA||state.loopB);const aTF=a.kind==='teeout';const inv=!lwF&&!aTF&&(state.conn.E==='N'||state.conn.N==='E');
  const lockA=elLock(l,c,j.idx),lockB=elLock(l,c,j.idx+1);
  const wireRow=w=>`<div class="conn"><div class="w"><i style="background:${WIRE[w].color};border:1px solid #999"></i>${a.id} ${WIRE[w].short} <span class="hint">(${clockText(wireGOf(a,w))})</span></div><span>→</span><select data-conn="${w}"><option value="E" ${state.conn[w]==='E'?'selected':''}>${b.id} étamé (${clockText(wireGOf(b,'E'))})</option><option value="N" ${state.conn[w]==='N'?'selected':''}>${b.id} cuivré (${clockText(wireGOf(b,'N'))})</option><option value="X" ${state.conn[w]==='X'?'selected':''}>non raccordé</option></select></div>`;
  return head(j.weldId,badge(j.status))+`<h3 style="margin-top:6px">Déclarer manchonnée</h3><label class="f">Manchonneur</label><input class="f" readonly value="${esc(me().name)}"><label class="f">Type de manchon</label><select class="f" id="f-manchon">${MANCHONS.map(p=>`<option value="${p[0]}">${p[1]}</option>`).join('')}</select>
   <h3>Raccordement des fils d'alarme (amont → aval)</h3><div class="card"><div class="muted" style="font-size:12px;margin-bottom:4px">Les deux bouts de tube vus de côté, l'œil un peu au-dessus : chaque fil sort de la mousse à son heure et rejoint en tendu la sortie d'en face en suivant l'acier (s'il fait le tour, il passe derrière : partie estompée). Touche un bout de fil du tube amont (à gauche), puis le bout d'en face qu'il rejoint (ou « ∅ non raccordé »).${state.wsel?` <b>Fil ${state.wsel==='E'?'étamé':'cuivré'} amont choisi → touche le fil aval.</b>`:''}</div>${amontAvalSVG(l,c,j)}${wiringSVG(a,b,state.conn,state.wsel,true,{A:state.loopA,B:state.loopB,selB:state.wselB})}${rotCtlHTML(a,'a','amont',lockA)}${rotCtlHTML(b,'b','aval',lockB)}${wireRow('E')}${wireRow('N')}${lwF?'<div class="okbox" style="background:#f3ecff;border-color:#c9b3ea">⟲ Fils bouclés ENTRE EUX — la boucle DH se ferme à ce manchon.</div>':inv?'<div class="err">Inversion : l\'étamé amont part sur le cuivré aval. Enregistre si c\'est ce qui a été fait — le manchon sera signalé « à reprendre ».</div>':'<div class="okbox">Raccordement droit : étamé ↔ étamé, cuivré ↔ cuivré.</div>'}
...
function collectStep(j,n){const s=j.steps[n]=j.steps[n]||{photos:[]};s.photos=s.photos||[];const q=id=>$('#'+id,sheetEl);
  ...
  return s;}
sheetEl.addEventListener('click',e=>{const b=e.target.closest('[data-act],[data-rot],[data-sw],[data-wire],[data-rotb],[data-flipb],[data-rota],[data-flipa],[data-saut],[data-teedown],[data-teedownj],[data-dhend],[data-stepok],[data-stepundo],[data-stkpick],[data-dhfrz],[data-gostep],[data-amvzoom]');if(!b)return;const s=state.sel;if(!s)return;const l=state.lines[s.line];
  if(b.dataset.dhfrz&&s.kind==='j'){e.preventDefault();showFrozenLoop(l,s.cond,l.cond[s.cond].joints[s.i]);return;}
  if(b.dataset.amvzoom&&s.kind==='j'){e.preventDefault();const j8=l.cond[s.cond].joints[s.i];
    openModal(`<h3 style="margin-top:0">${esc(j8.weldId)} — l'endroit, vu large</h3>${amontAvalSVG(l,s.cond,j8,true)}
     <div class="actions" style="margin-top:8px"><button class="btn primary block" id="amv-plan">Ouvrir le PLAN à cet endroit</button><button class="btn block" data-close>Fermer</button></div>`);
    const bp8=$('#amv-plan');if(bp8)bp8.onclick=()=>{const p8=posAtChainage(l,l.cond[s.cond].els[j8.idx].m1);closeModal();closeSheet();state.tab='plan';renderAll();centerOn(p8.x,p8.y,Math.max(state.view.k,30));};
    return;}
  ...
  if(b.dataset.stkpick!==undefined){const md6=b.dataset.stkneed,val6=b.dataset.stkpick;
    sheetEl.querySelectorAll('[data-stkpick][data-stkneed="'+md6+'"]').forEach(x=>{const on=x.dataset.stkpick===val6;
      ...
      if(x.dataset.stkhit!==undefined)return; // grande cible tactile invisible de la carte : pas de style
  ...
  if(stp&&s.kind==='j'){e.preventDefault();const j2=l.cond[s.cond].joints[s.i];const n=+stp;j2.steps=j2.steps||{};
    if(b.dataset.stepundo){ // annuler POUR DE VRAI (retour Ethan 25/08 : l'étape revenait toute seule — migrateSteps la recochait depuis le statut resté en place)
      ...
      if(n===2){j2.wire='a_raccorder';j2.conn={E:'E',N:'N'};delete j2.loopA;delete j2.loopB;j2.cont=false;j2.iso=false;j2.isoVal='';state.conn={E:'E',N:'N'};state.loopA=false;state.loopB=false;state.wsel=null;state.wselB=null;} // fils dé-déclarés, bouclage figé parti avec l'étape
      ...
      if(n===2){ // les fils déclarés au schéma deviennent ceux de la soudure, ET on FIGE le bouclage de l'instant t
        const lw2=!!(state.loopA||state.loopB);const aT9=l.cond[s.cond].els[j2.idx];const teeOut9=!!(aT9&&aT9.kind==='teeout');const inv2=!lw2&&!teeOut9&&(state.conn.E==='N'||state.conn.N==='E');j2.conn=lw2?{E:'X',N:'X'}:{...state.conn};if(state.loopA)j2.loopA=true;else delete j2.loopA;if(state.loopB)j2.loopB=true;else delete j2.loopB;j2.wire=inv2?'inversion':'raccorde';
        ...
        if(dj2){const AP3=dhAtPoint(dj2.D,dj2.row);const cc3=[AP3.up.closed&&AP3.up.R!==null?{dir:'amont',...AP3.up}:null,AP3.down.closed&&AP3.down.R!==null?{dir:'aval',...AP3.down}:null].filter(Boolean);
          ...
          j2.steps[2].dh={at:new Date().toISOString(),meas:mv||null,iso:j2.steps[2].iso!=null?j2.steps[2].iso:null,
            dir:bst?bst.dir:null,closure:bst?(bst.self?'boucle amont entière (départ → ce manchon)':bst.kind==='loopSelf'?'fils bouclés entre eux à CE manchon (boucle départ → ici)':bst.kind==='loopW'?'fils bouclés entre eux au manchon '+bst.row.weldId:bst.kind==='temp'?'pont ⟲ '+bst.row.weldId:'bout bouclé ('+(bst.state==='coiffe'?'dans la coiffe':'sortie de coiffe')+')'):null,
            closureId:bst?(bst.self||bst.kind==='loopSelf'?'self':bst.kind==='temp'||bst.kind==='loopW'?bst.row.weldId:'bout'):null,closureLine:bst&&(bst.kind==='temp'||bst.kind==='loopW')?bst.row.line:null,closureIdx:bst&&(bst.kind==='temp'||bst.kind==='loopW')?bst.row.idx:null,
  ...
  if(b.dataset.wire){const [side,w]=b.dataset.wire.split(':');snapshotForm();
    if(side==='a'){ // côté AMONT : 1er tap = choisir le fil ; 2e tap sur L'AUTRE fil amont = les boucler entre eux (Ethan 26/08)
      if(state.loopA){state.loopA=false;if(!state.loopB)state.conn={E:'E',N:'N'};toast('Bouclage amont défait — fils à raccorder');}
      else if(state.wsel&&state.wsel!==w){state.loopA=true;state.wsel=null;state.conn={E:'X',N:'X'};toast('⟲ Fils AMONT bouclés entre eux à ce manchon');}
      else state.wsel=state.wsel===w?null:w;}
    else if(state.wsel){state.conn[state.wsel]=w;state.wsel=null;}
    else if(w==='X'){state.conn.E='X';state.conn.N='X';state.loopA=false;state.loopB=false;state.wselB=null;}
    else{ // côté AVAL sans fil amont choisi : toucher ses DEUX fils l'un après l'autre = bouclés entre eux
      if(state.loopB){state.loopB=false;if(!state.loopA)state.conn={E:'E',N:'N'};state.wselB=null;toast('Bouclage aval défait — fils à raccorder');}
      else if(state.wselB&&state.wselB!==w){state.loopB=true;state.wselB=null;state.conn={E:'X',N:'X'};toast('⟲ Fils AVAL bouclés entre eux à ce manchon');}
      else state.wselB=state.wselB===w?null:w;}
    renderSheet();return;}
  ...
  if(a.startsWith('form-')){state.sheetMode=a;state.wsel=null;state.wselB=null;resetForm();const j=l.cond[s.cond].joints[s.i];state.conn={...j.conn};state.loopA=!!j.loopA;state.loopB=!!j.loopB;state.sw={cont:false,iso:false,etanch:false,mousse:false};renderSheet();return;}
  ...
  if(b.dataset.teedown!==undefined){const lk=elLock(l,s.cond,s.i);if(lk)return lockedToast(lk);const el=l.cond[s.cond].els[s.i];el.teeDown=b.dataset.teedown==='1';recordElPos(l,s.cond,el);renderSheet();renderPlan();toast(el.teeDown?el.id+' : té retourné — les fils passent EN DESSOUS (dessinés en pointillé)':el.id+' : fils au-dessus');return;}
  if(b.dataset.teedownj&&s.kind==='j'){const j9=l.cond[s.cond].joints[s.i];const off9=b.dataset.teedownj==='b'?1:0;const el9=l.cond[s.cond].els[j9.idx+off9];if(!el9)return;const lk9=elLock(l,s.cond,j9.idx+off9);if(lk9)return lockedToast(lk9);el9.teeDown=!el9.teeDown;recordElPos(l,s.cond,el9);renderSheet();renderPlan();toast(el9.teeDown?el9.id+' : té retourné — les fils passent EN DESSOUS':el9.id+' : fils au-dessus');return;}
  ...
  if(a==='save-manchon'){if(!state.sw.etanch){snapshotForm();state.err='Le test d\'étanchéité doit être validé (ou signale un problème).';renderSheet();return;}if(!state.pendingPhotos.length){snapshotForm();state.err='Ajoute une photo du raccordement des fils et du manchon.';renderSheet();return;}
    ...
    const lwm=!!(state.loopA||state.loopB);const aT8=l.cond[s.cond].els[j.idx];const teeOut8=!!(aT8&&aT8.kind==='teeout');const inv=!lwm&&!teeOut8&&(state.conn.E==='N'||state.conn.N==='E');j.conn=lwm?{E:'X',N:'X'}:{...state.conn};if(state.loopA)j.loopA=true;else delete j.loopA;if(state.loopB)j.loopB=true;else delete j.loopB;j.cont=!!state.sw.cont;j.iso=!!state.sw.iso;j.isoVal=val('f-iso')||'';j.wire=inv?'inversion':'raccorde';if(inv)j.note='Inversion enregistrée au manchonnage';
...
sheetEl.addEventListener('change',e=>{if(e.target.id==='teeMode'){const s0=state.sel;if(!s0||s0.kind!=='j')return;const j=state.lines[s0.line].cond[s0.cond].joints[s0.i];j.tee={...(j.tee||{}),mode:e.target.value||undefined};delete j.tee.wire;sync.saveWeld(state.siteId,{...j}).then(ok=>{if(ok)setCloudBadge('enregistré '+new Date().toLocaleTimeString('fr-FR'));});toast('Raccordement de l\'antenne enregistré');return;}const sph=e.target.closest('[data-stepph]');
...
function wirePath(lineId,cond,wire){const line=state.lines[lineId];const segs=[];let d=0,cur=wire;if(!line.cond[cond])return {segs,total:0};if(typeof linkTraceurBranches==='function')linkTraceurBranches();const {els,joints}=line.cond[cond];
  els.forEach((e,i)=>{const brId=e.branchLine||(typeof e.branch==='string'?e.branch:null);const am=brId&&state.lines[brId]?antennaMode(brId,cond,e):null;if(brId&&am&&am.mode==='serie'&&cur===am.wire&&state.lines[brId].cond[cond]){segs.push({line:lineId,elIdx:i,kind:'main',m0:d,m1:d+e.len/2,phys0:e.m0,phys1:e.m0+e.len/2,w:cur});d+=e.len/2;const br=state.lines[brId];const bl=br.cond[cond].els;const bj2=br.cond[cond].joints;const wOpen=j2=>!!(j2&&(j2.wire==='a_raccorder'||j2.loopA||j2.loopB||(j2.conn&&j2.conn.E==='X'&&j2.conn.N==='X')));bl.forEach((be,bi)=>{segs.push({line:brId,elIdx:bi,kind:'branchOut',m0:d,m1:d+be.len,phys0:be.m0,phys1:be.m1,w:cur});d+=be.len;if(bi<bl.length-1&&wOpen(bj2[bi]))segs.push({line:brId,elIdx:bi,kind:'cut',m0:d,m1:d,phys0:be.m1,phys1:be.m1,open:true});});for(let bi=bl.length-1;bi>=0;bi--){const be=bl[bi];if(bi<bl.length-1&&wOpen(bj2[bi]))segs.push({line:brId,elIdx:bi,kind:'cut',m0:d,m1:d,phys0:be.m1,phys1:be.m1,open:true});segs.push({line:brId,elIdx:bi,kind:'branchBack',m0:d,m1:d+be.len,phys0:be.m1,phys1:be.m0,w:cur});d+=be.len;}segs.push({line:lineId,elIdx:i,kind:'main',m0:d,m1:d+e.len/2,phys0:e.m0+e.len/2,phys1:e.m1,w:cur});d+=e.len/2;}
    else{segs.push({line:lineId,elIdx:i,kind:'main',m0:d,m1:d+e.len,phys0:e.m0,phys1:e.m1,w:cur});d+=e.len;}
    const j=joints[i];if(j&&j.wire!=='a_raccorder'){const to=(j.loopA||j.loopB)?'X':j.conn[cur];if(to==='X'){segs.push({line:lineId,elIdx:i,kind:'cut',m0:d,m1:d,phys0:e.m1,phys1:e.m1});return;}cur=to;}else if(j){segs.push({line:lineId,elIdx:i,kind:'cut',m0:d,m1:d,phys0:e.m1,phys1:e.m1,open:true});}});
  return {segs,total:d};}
function locate(lineId,cond,wire,dist,fromD){const {segs,total}=wirePath(lineId,cond,wire);
  const f0=+fromD||0;const lo3=Math.min(f0,dist),hi3=Math.max(f0,dist);
  const cut=segs.find(x=>x.kind==='cut'&&x.m0>lo3+.02&&x.m0<hi3-.02);
  if(cut){const jj2=state.lines[cut.line]&&state.lines[cut.line].cond[cond]&&state.lines[cut.line].cond[cond].joints[cut.elIdx];return {ok:false,total,beyondCut:true,cutWeld:jj2?jj2.weldId:null,cutAt:cut.m0};}
  if(dist>total)return {ok:false,total};const s=segs.find(x=>dist>=x.m0&&dist<=x.m1);if(!s)return {ok:false,total};const e=state.lines[s.line].cond[cond].els[s.elIdx];const f=(dist-s.m0)/Math.max(.001,s.m1-s.m0);const phys=s.phys0+(s.phys1-s.phys0)*f;return {ok:true,total,dist,seg:s,e,phys,fromJ:phys-e.m0,toJ:e.m1-phys,where:s.kind==='branchOut'?'dans l\'antenne (vers la SST)':s.kind==='branchBack'?'dans l\'antenne (retour vers le té)':'sur la ligne',lineName:state.lines[s.line].name,line:s.line};}
...
const DH_DEF={rkm:12.5,isoMin:200,tol:5,piqL:2};
function loadDH(){let o={};try{o=JSON.parse(localStorage.getItem('trace:dh')||'{}')||{};}catch(e){}return Object.assign({},DH_DEF,o,{at:null});}
state.dh=loadDH();
function saveDH(){try{localStorage.setItem('trace:dh',JSON.stringify({rkm:state.dh.rkm,isoMin:state.dh.isoMin,tol:state.dh.tol,piqL:state.dh.piqL}));}catch(e){}}
...
// Té retourné (vidange, saut passant dessous) : MÊME fil dans la branche — seule la position des fils change de côté (Ethan 26/08).
function wireDefFournisseur(){const sup=(NET&&NET.supplier)||'';return sup==='LOGSTOR'?'E':'N';} // RÈGLE FIXE, non réglable (Ethan 26/08) : Renalia / AXIOM = CUIVRÉ (nu), LOGSTOR = étamé — l'ancien réglage « fil dans la branche » pouvait forcer l'étamé et créer des faux plans
function wireOfTee(e){return wireDefFournisseur();}
// côté de la branche d'un té par rapport à son axe dessiné (convention d'offsetPoly : normale = (ty,−tx)) — partagé plan / fiche manchon
function teeSideSign(e){const brT=e&&e.kind==='tee'&&Array.isArray(e.branch)&&e.branch.length===2?e.branch:null;if(!brT)return 0;
  const pl0=e.axis&&e.axis[0];if(!pl0||pl0.length<2)return 0;let bi=1,bd=1e9;
...
// raccordement d'une antenne au té : mode (serie = insérée dans la boucle de la parente ; boucle = bouclée sur elle-même au manchon de sortie ; none = pas encore raccordée), porté par la soudure de sortie de té (j.tee). Le FIL n'est PAS réglable : règle fournisseur, point — un ancien j.tee.wire stocké est ignoré (Ethan 26/08).
function antennaMode(childId,cond,parentEl){const L=state.lines[childId];const cd=L&&L.cond[cond];const tj=cd&&cd.joints[0];const t=(tj&&tj.tee)||{};
  let mode=t.mode||'serie';
  if(tj&&(tj.loopA||tj.loopB))mode='boucle'; // fils bouclés ENTRE EUX au manchon de sortie : l'antenne est ISOLÉE (bouclée à sa tête) et le fil de la parente TRAVERSE le té — le pont assure sa continuité, ça ne ferme PAS la boucle de la parente (cas S-2505 d'Ethan, 26/08)
  return {mode,wire:wireOfTee(parentEl||null),j:tj};}
...
function dhLoop(lineId,cond){const line=state.lines[lineId];if(!line||!line.cond[cond])return null;linkTraceurBranches();
  ...
  const walk=(wire)=>{const segs=[];const cuts=[];let d=0,cur=wire;const rec=(lid,depth)=>{const L=state.lines[lid];const cd=L&&L.cond[cond];if(!cd)return;const {els,joints}=cd;
      els.forEach((e,i)=>{const br=e.branchLine||((typeof e.branch==='string')?e.branch:null);const child=br&&state.lines[br]&&state.lines[br].cond[cond]&&depth<4?br:null;
        ...
        const j=joints[i];if(j){segs.push({line:lid,elIdx:i,kind:'joint',m0:d,m1:d,j});if(j.wire!=='a_raccorder'){const to=(j.loopA||j.loopB)?'X':(j.conn||{})[cur];if(to==='X'){segs.push({line:lid,elIdx:i,kind:'cut',m0:d,m1:d});cuts.push(d);}else if(to)cur=to;}}});};
  ...
  const add=(segs,isE)=>{segs.forEach(s=>{if(s.kind!=='joint'&&s.kind!=='jointOut')return;const key=s.line+'|'+s.elIdx;let r=byKey[key];
    if(!r){const j=s.j;const L=state.lines[s.line];const e=L.cond[cond].els[s.elIdx];r=byKey[key]={weldId:j.weldId,line:s.line,lineName:L.name,idx:s.elIdx,pk:e?e.m1:0,wire:j.wire||'a_raccorder',status:j.status,loopA:!!j.loopA,loopB:!!j.loopB,dE:null,dN:null,R:null,antenna:s.kind==='jointOut',iso:j.iso,isoVal:j.isoVal,open:false};
...
function dhAtPoint(D,row){const i=D.rows.indexOf(row);if(i<0)return null;
  ...
  const mk=(r)=>{ // fermeture au pont ⟲ posé à la soudure r (raccordée ou non : le pont relie les fils là)
    if(row.dE===null||r.dE===null||row.dN===null||r.dN===null)return {closed:true,kind:'temp',row:r,dE:(row.dE!==null&&r.dE!==null)?+Math.abs(row.dE-r.dE).toFixed(1):null,dN:(row.dN!==null&&r.dN!==null)?+Math.abs(row.dN-r.dN).toFixed(1):null,R:null,antenne:true}; // un des deux fils ne passe pas par les deux points : pas de valeur calculable (jamais de soustraction avec null — bug 0,73 Ω d'Ethan)
  ...
  const mkc=(r,kind)=>{const m=mk(r);if(m.closed)m.kind=kind;return m;}; // même calcul, autre nature de fermeture (fils bouclés entre eux au manchon r)
  const dir=(step)=>{
    if(step<0&&row.loopA&&row.dE!==null&&row.dN!==null)return {closed:true,kind:'loopSelf',row,dE:row.dE,dN:row.dN,R:row.R}; // les fils amont sont pontés ICI : boucle amont entière (départ → ce manchon)
    for(let j2=i+step;j2>=0&&j2<D.rows.length;j2+=step){const r=D.rows[j2];
      if(step<0&&r.loopB)return mkc(r,'loopW'); // en remontant : un manchon dont les fils AVAL (face à nous) sont pontés ferme la boucle
      if(step>0&&r.loopA)return mkc(r,'loopW'); // en descendant : pareil avec les fils AMONT pontés
...
function dhDirLab(d){if(!d)return '—';
  if(d.closed)return d.kind==='loopW'?'fermée — fils bouclés entre eux au manchon '+esc(d.row.weldId):d.kind==='loopSelf'?'fermée ICI — fils amont bouclés entre eux (boucle départ → ce manchon)':d.kind==='temp'?(d.antenne?'fermée au pont ⟲ '+esc(d.row.weldId)+' (antenne : calcul non géré)':'fermée au pont ⟲ '+esc(d.row.weldId)):'fermée au bout ('+(d.state==='coiffe'?'bouclé dans la coiffe':'sortie de coiffe')+')';
...
function renderBouclage(){const el=$('#bouclage');const L=state.locate;const mains=Object.values(state.lines).filter(l=>!l.parent||['A','R'].some(c=>l.cond[c]&&antennaMode(l.id,c).mode==='boucle'));if(!mains.length){el.innerHTML='<h2 class="vt">DH — détection d\'humidité</h2><div class="card muted">Aucune ligne dans ce chantier — trace un réseau avec le traceur.</div>';return;}if(!mains.find(l=>l.id===L.line))L.line=mains[0].id;const line=state.lines[L.line];
  ...
  if(!useAt)res=locate(L.line,cond,L.wire,+L.d||0);
    ...
    if(refBase===null)res={ok:false,noWire:true};
      ...
      res=dFil<0?{ok:false,uphill:true,total:refBase}:locate(L.line,cond,L.wire,dFil,refBase);}}
  ...
  if(atRow){const AP=dhAtPoint(D,atRow);const up=AP.up,dn=AP.down;const tol=+state.dh.tol||5;
    ...
    const dirCard=(key,lab,d)=>{const okc=d.closed&&d.R!==null;const on=state.dh.dir===key;
      ...
      return `<div class="dhdir ${on?'on':''}" data-dhdir="${key}"><div class="dt">${lab}${on?' <span style="color:#eb6834">● celle que je mesure</span>':''}</div><div class="dr">${fmt2(d.R)} Ω <span class="du">attendu</span></div><div class="dm">${dhDirLab(d)}<br>étamé ${fmt(d.dE)} m + cuivré ${fmt(d.dN)} m = ${fmt(d.dE+d.dN)} m de fil</div></div>`;};
    ...
    if(lv){
      if(!act)locBox=`<div class="err" style="margin-top:6px">Localisation : aucune boucle fermée depuis ce point (pose un ⟲ ou déclare un bout bouclé).</div>`;
        ...
        if(isFinite(x)&&x>0){if(x>Lb+0.5)locBox=`<div class="err" style="margin-top:6px">Distance au-delà de la boucle ${act.dir} (${fmt(Lb)} m de fil au total).</div>`;
            ...
            const res2=locate(L.line,cond,wire2,dFil,x<=act.dE?atRow.dE:closeDN);
            ...
            if(res2.ok)locBox=`<div class="warnbox" style="margin-top:6px"><b>Défaut estimé à ${fmt(x)} m de fil du point de mesure</b> (boucle ${act.dir}, fil ${wire2==='E'?'étamé':'cuivré'}) : ≈ <b>${esc(res2.e.id)}</b>, ${esc(res2.where)} — ${esc(res2.lineName)}, PK ${fmt(res2.phys)} m. <a href="#" id="dh-locshow" style="color:#1c3d6b;font-weight:700">Voir sur le plan (trajet + zone)</a></div>`;
            else locBox=res2.beyondCut?`<div class="err" style="margin-top:6px">✋ Fil COUPÉ avant cette distance${res2.cutWeld?' — manchon '+esc(res2.cutWeld)+' non raccordé / bouclé':''} : le défaut ne peut pas être au-delà. Raccorde (ou vérifie) ce manchon d'abord.</div>`:`<div class="err" style="margin-top:6px">Distance au-delà du fil connu (${fmt(res2.total)} m).</div>`;
    ...
    const calc=act?`<details class="muted" style="font-size:12px;margin-top:6px"><summary style="cursor:pointer;color:#52514e">Le calcul, pas à pas (boucle ${act.dir})</summary>
      ...
      · le courant fait l'aller par un fil et le retour par l'autre : fil étamé <b>${fmt(act.dE)} m</b> + fil cuivré <b>${fmt(act.dN)} m</b> = <b>${fmt(act.dE+act.dN)} m de fil</b><br>
  ...
  el.innerHTML=`<h2 class="vt">DH — détection d'humidité</h2>
   ...
   <div class="card"><h3 style="margin-top:0;color:${colC}">Boucle ${cond==='A'?'aller':'retour'} — ${esc(line.name)}</h3>
     ...
     <div style="overflow:auto;max-height:52vh;border:1px solid var(--line);border-radius:8px"><table class="rc" id="dh-rows" style="margin:0"><tr><th>Soudure</th><th>PK</th><th>Fils</th><th title="longueur électrique cumulée du fil étamé depuis le départ">étamé</th><th title="fil cuivré cumulé">cuivré</th><th title="résistance de boucle attendue au départ si on ponte étamé ↔ nu à ce manchon">Ω pont.</th></tr>${rowsHTML}</table></div>
     <div class="muted" style="font-size:11.5px;margin-top:4px">Ligne surlignée = fin actuelle de la boucle. Lignes grisées = au-delà du premier manchon non raccordé. « ↳ » = soudure d'antenne, prise en série sur le fil qui plonge au té (aller-retour compté).</div></div>
   ...
   <div class="row" style="display:flex;gap:6px;flex-wrap:wrap;align-items:end"><div style="flex:none"><label class="f">Branché à</label><select class="f" id="loc-from" style="min-width:132px">${atRow?`<option value="at" ${useAt?'selected':''}>${esc(atRow.weldId)} · choisi</option>`:''}<option value="start" ${!useAt?'selected':''}>${esc(line.start.split(' — ')[0])} · départ</option></select></div>${useAt?`<div style="flex:none"><label class="f">Je vise</label><select class="f" id="loc-dir" style="min-width:92px"><option value="down" ${L.dir!=='up'?'selected':''}>l'aval</option><option value="up" ${L.dir==='up'?'selected':''}>l'amont</option></select></div>`:''}<div style="flex:none"><label class="f">Fil</label><select class="f" id="loc-wire" style="min-width:86px"><option value="E" ${L.wire==='E'?'selected':''}>Étamé</option><option value="N" ${L.wire==='N'?'selected':''}>Cuivré</option></select></div><div><label class="f">Distance (m)</label><input class="f" id="loc-d" type="number" inputmode="decimal" step="0.1" value="${L.d}" style="width:96px"></div><button class="btn primary" id="loc-go" style="height:38px">Localiser</button></div>
   ${res.ok?`<div class="okbox" style="margin-top:10px"><b>≈ ${res.e.id} (${res.e.type==='barre'?'barre '+fmt(res.e.len)+' m':res.e.kind||res.e.type}), ${res.where}</b> — à ${fmt(res.fromJ)} m du manchon amont, ${fmt(res.toJ)} m du manchon aval · ${esc(res.lineName)} (PK ${fmt(res.phys)} m)${useAt?` · <span class="dim">${fmt(+L.d||0)} m depuis ${esc(atRow.weldId)} vers ${L.dir==='up'?"l'amont":"l'aval"} sur ${L.wire==='E'?"l'étamé":'le cuivré'}</span>`:''}. <a href="#" id="loc-show" style="color:#1c3d6b">Voir sur le plan</a></div>`
    :res.noWire?`<div class="err" style="margin-top:10px">Le fil cuivré ne passe pas par ce manchon — branche-toi sur l'étamé.</div>`
    ...
    :res&&res.beyondCut?`<div class="err" style="margin-top:10px">✋ Fil COUPÉ avant cette distance${res.cutWeld?' — manchon '+esc(res.cutWeld)+' non raccordé / bouclé':''} : le défaut ne peut pas être au-delà. Raccorde (ou vérifie) ce manchon d'abord.</div>`
...
function wireTraceSVG(lineId,cond,legs,k,color,mode){const col=color||'#e8102d';
  const groups=[];
  legs.forEach((leg,li)=>{const wp=wirePath(lineId,cond,leg.wire);let lo=Math.min(leg.d0,leg.d1),hi=Math.max(leg.d0,leg.d1);
    // fils NON SERTIS (manchon à raccorder / bouclé) = circuit ouvert : le défaut ne se propage pas au-delà (Ethan 26/08)
    const cuts=wp.segs.filter(sg=>sg.kind==='cut'&&sg.m0>lo+.02&&sg.m0<hi-.02).map(sg=>sg.m0);
    if(cuts.length){if(leg.d1>=leg.d0)hi=Math.min(hi,Math.min(...cuts));else lo=Math.max(lo,Math.max(...cuts));}
  ...
  groups.forEach(g2=>{const L3=state.lines[g2.line];const cd=L3&&L3.cond&&L3.cond[cond];if(!cd)return;
    ...
    const isBr=g2.kind==='branchOut'||g2.kind==='branchBack';
    // dans l'antenne, le fil suivi n'est PAS forcément la même lettre que sur la parente : c'est le manchon de SORTIE DE TÉ
    // qui dit sur quel fil de l'antenne le brin amont est serti (sa conn) — sinon le rouge saute sur le mauvais fil et croise (Ethan 26/08)
    let wOut=g2.w,wBack=opp(g2.w);
    if(isBr){const j0=cd.joints[0];const cE=j0&&j0.conn&&j0.conn.E;wOut=(cE==='E'||cE==='N')?cE:'E';wBack=opp(wOut);}
    const ppm=L3.ppm||1;const flw=Math.max(1.4/k,.022*ppm);let first=null,last=null;
    const glow=col==='#8a2be2'?'#c58cf0':'#ff5f5f';
    cd.els.forEach(e=>{if(e.m1<=lo2||e.m0>=hi2)return;const pl=e.axis&&e.axis[0];if(!pl||pl.length<2)return;const wn=isBr?(g2.kind==='branchOut'?wOut:wBack):g2.w;
...
function renderStock(){const el=$('#stock');const s=stockOf();
  ...
  el.querySelectorAll('[data-stkdel]').forEach(b=>b.onclick=()=>{const z=stockZoneById(b.dataset.stkdel);if(!z)return;const agg=zoneAgg(s,z.id);const reste=agg.reduce((t,a)=>t+a.reste,0);const pend2=s.lots.some(l2=>l2.zone===z.id&&l2.pend);
    if(reste>0||pend2){openStockZoneDelete(z.id);return;} // il reste des pièces : on demande OÙ les basculer — jamais de disparition silencieuse (Ethan 26/08)
...
/* ----- suppression d'une zone qui a encore du stock : on bascule d'abord les pièces (Ethan 26/08) ----- */
function openStockZoneDelete(zoneId){const s=stockOf();const z=stockZoneById(zoneId);if(!s||!z)return;
  const agg=zoneAgg(s,z.id).filter(a=>a.reste>0);const reste=agg.reduce((t,a)=>t+a.reste,0);
  const nPend=s.lots.filter(l2=>l2.zone===z.id&&l2.pend).length;
  const others=s.zones.filter(z2=>z2.id!==zoneId&&z2.status==='ok');
  openModal(`<h3 style="margin-top:0">Supprimer « ${esc(z.name)} »</h3>
   <div class="warnbox">Il reste <b>${reste} pièce(s)</b>${nPend?' + '+nPend+' lot(s) attendu(s)':''} dans cette zone : bascule-les d'abord.</div>
   ${agg.length?`<div style="overflow:auto;max-height:30vh"><table class="rc" style="margin-top:6px">${agg.map(a=>`<tr><td>${esc(a.label)}</td><td style="text-align:right"><b>${stkFmtQ(a.reste)}</b></td></tr>`).join('')}</table></div>`:''}
   <label class="f" style="margin-top:8px">Tout basculer vers</label>
   <select class="f" id="stkzd-dest">${others.map(z2=>`<option value="${z2.id}">${esc(z2.name)}</option>`).join('')}</select>
   ${others.length?'':'<p class="hint">Aucune autre zone : décharge un camion ou scinde vers une nouvelle zone d\'abord.</p>'}
   <div class="actions" style="margin-top:8px"><button class="btn primary block" id="stkzd-ok" ${others.length?'':'disabled'}>Basculer tout, puis supprimer la zone</button>
    <button class="btn block" id="stkzd-force" style="color:#d03b3b">Supprimer SANS basculer (les pièces disparaissent du suivi)</button>
    <button class="btn block" data-close>Annuler</button></div>`);
  const wipe2=()=>{s.zones=s.zones.filter(z2=>z2.id!==zoneId);saveStock();closeModal();renderStock();renderPlan();};
  const ok2=$('#stkzd-ok');if(ok2)ok2.onclick=()=>{const destId=$('#stkzd-dest').value;if(!destId)return;
    const lvsByKey={};s.lots.forEach(l=>{if(l.zone===zoneId&&l.liv)(lvsByKey[l.key]=lvsByKey[l.key]||new Set()).add(l.liv);});
    agg.forEach(a=>s.moves.push({at:new Date().toISOString(),by:(me()||{}).name||state.userId,label:a.label,key:matchKey(a),livs:[...(lvsByKey[a.key]||[])],qty:a.reste,from:zoneId,to:destId})); // la trace du parcours suit chaque référence
    s.lots.forEach(l=>{if(l.zone!==zoneId)return;const ex=s.lots.find(l2=>l2!==l&&l2.zone===destId&&l2.key===l.key&&l2.liv===l.liv&&!l2.pend);if(ex&&!l.pend){ex.qty+=l.qty;l.qty=0;}else l.zone=destId;}); // livré ET pris déménagent ensemble : les comptes restent justes
    s.lots=s.lots.filter(l=>l.qty>0||l.pend);
    s.takes.forEach(t=>{if(t.zone===zoneId)t.zone=destId;});
    wipe2();toast('Basculé vers « '+((stockZoneById(destId)||{}).name||'')+' » — zone supprimée');};
  const fc2=$('#stkzd-force');if(fc2)fc2.onclick=()=>{if(!confirm('Vraiment supprimer « '+z.name+' » AVEC ses '+reste+' pièce(s) ? Elles disparaissent du suivi.'))return;
    s.lots=s.lots.filter(l2=>l2.zone!==zoneId);s.takes=s.takes.filter(t=>t.zone!==zoneId);wipe2();toast('Zone supprimée');};}
